            ...
            for (i = 1; i < lines.length; i++) {
                ...
                if (row.length >= numColumns) {
                    ...
                    features.push(new atlas.data.Feature(new atlas.data.Point([parseFloat(row[header['longitude']]), parseFloat(row[header['latitude']])]), {
                        ...
                        district: row[header['district']],
                        division: row[header['division']],
                        location: row[header['location']],
                        sub_location: row[header['sub_location']],
                        level: row[header['level']],
                        agency: row[header['agency']],
                        email: row[header['email']],
                        emergency_number_one: row[header['emergency_number_one']],
                        emergency_number_two: row[header['emergency_number_two']],
                    }));
                }
            }
...
//Convert the closing time into a nicely formated time.
        ...
        '<br/>', distance,
        ' miles away',
        '<br/><form action="sendsms?hospital_phone=+254795339084&lat=0.22&lon=1.33><div class="col col-auto"><button type="submit" class="btn btn-primary btn-lg call-patient-button">Emergency Response</button></div></form>'
    );
...
function getAddressLine2(properties) {
    var html = [properties['name']];

    if (properties['province']) {
        html.push(', ', properties['province']);
    }

    if (properties['district']) {
        html.push(', ', properties['district']);
    }
    ...
    if (properties['sub_location']) {
        html.push(' ', properties['sub_location']);
    }
